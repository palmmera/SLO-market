...
function typeLabel(type: CollectionType) {
  ...
  if (type === "PRODUCE_STAND") return "Produce stand";

  if (type === "YARD_SALE") return "Yard sale";

  if (type === "ROOM") return "Room sale";
...
export function GarageSaleRow({ sale }: { sale: PhotoSaleRowData }) {

  const router = useRouter();

  const [pending, start] = useTransition();

  const isProduce = sale.collectionType === "PRODUCE_STAND";
  ...
  const editHref = isProduce

    ? `/sell/food/photo/${sale.id}?${editQs.toString()}`

    : `/sell/photo/${sale.id}?${editQs.toString()}`;
    ...
    <div className="flex items-center gap-3 rounded-2xl bg-white p-3 card-shadow">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-sm text-muted">
          ...
          {typeLabel(sale.collectionType)} · {sale.itemCount} item{sale.itemCount === 1 ? "" : "s"}
